...
export default function HotspotPortalClient({
  routerId,
  initialPlans,
  initialPortalConfig,
  initialBranding,
  ...
  // Data state — initialised from server props
  ...
  const [loading, setLoading] = useState(initialPlans.length === 0 && !fetchFailed)
  const [error, setError] = useState<string | null>(fetchFailed ? "Failed to load plans" : null)
  ...
  // ── 🔥 OPTIMIZATION: Only fetch if server didn't provide plans ──
  useEffect(() => {
    // If we already have plans from server, skip network call
    if (initialPlans.length > 0 || fetchFailed) {
      return
    }

    // Otherwise, fetch (fallback for cold cache / error)
  ...
  }, [routerId, initialPlans.length, fetchFailed])
