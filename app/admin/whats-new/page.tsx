  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      <div className="text-center mb-12">
        ...
        <h1 className="text-3xl font-black text-slate-900 tracking-tight">What's New in Netily</h1>
        ...
        {logs.length === 0 ? (
          ...
          logs.map((log) => (
            <div key={log.id} className="relative pl-8 md:pl-0">
              ...
              <div className="hidden md:block absolute left-[120px] top-0 bottom-[-48px] w-px bg-slate-200" />

              <div className="md:flex gap-8 relative">
                {/* Date / Version */}
                <div className="md:w-[100px] flex-shrink-0 pt-1 text-left md:text-right mb-2 md:mb-0">
                  <p className="text-sm font-bold text-slate-900">{new Date(log.release_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                  {log.version && <p className="text-xs font-mono text-slate-400 mt-1">{log.version}</p>}
                </div>

                {/* Timeline Node Icon */}
                <div className="hidden md:flex absolute left-[120px] -translate-x-1/2 w-8 h-8 rounded-full bg-white border-2 border-slate-100 items-center justify-center z-10 shadow-sm">
                  {getIcon(log.update_type)}
                </div>

                {/* Content Card */}
                <div className="flex-grow bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="md:hidden">{getIcon(log.update_type)}</span>
                    <h2 className="text-xl font-bold text-slate-900">{log.title}</h2>
                  </div>
